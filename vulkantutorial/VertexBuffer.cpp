...
void VertexBuffer::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size)
{
    ...
    vkAllocateCommandBuffers(mainApp->device, &allocInfo, &commandBuffer);
    ...
    vkQueueSubmit(mainApp->graphicsQueue, 1, &submitInfo, VK_NULL_HANDLE);
    vkQueueWaitIdle(mainApp->graphicsQueue);

    vkFreeCommandBuffers(mainApp->device, mainApp->commandPool, 1, &commandBuffer);
}
